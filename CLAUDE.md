...
## Règle transversale : jamais de contenu réel absent du HTML servi par le serveur

Ce bug est apparu 3 fois de suite sous des formes différentes (réponses FAQ démontées du DOM, étapes de "Comment ça marche" non montées, compteurs `CountUp` affichant "0" côté serveur) — assez pour en faire une règle explicite plutôt que de continuer à le corriger au cas par cas. **Tout composant qui affiche une valeur/un texte réel après une animation, une interaction ou un effet client (`useEffect`, montage conditionnel, compteur animé) doit rendre la valeur finale réelle dans le HTML généré côté serveur**, et n'utiliser le JS que pour l'effet visuel progressif (repli CSS, animation qui part de la vraie valeur affichée plutôt que d'un placeholder à 0/vide). Ne jamais faire dépendre la présence du contenu réel de l'exécution du JS côté client. À vérifier systématiquement pour tout nouveau composant animé ajouté au site.
